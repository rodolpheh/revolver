...
class Revolver(object):
  ...
  @property
  def hourly(self):
    return "{}.{:02d}.{:02d}.{}{:02d}".format(self.year, self.month, self.day, self.hour, self.minute)

  def __str__(self):
    return self.hourly

  def __repr__(self):
    return "Revolver(year={}, month={}, day={}, hour={}, minute={})".format(self.year, self.month, self.day, self.hour, self.minute)
  ...
  def from_iso(iso_date):
    if iso_date.tzinfo is None or iso_date.tzinfo.utcoffset(iso_date) is None:
        #iso_date = iso_date.replace(tzinfo=pytz.timezone('Europe/London'))
    ...
    month = metric_time.RepublicanCalendar().MONTHS.index(date.month) + 1

    revolver = Revolver(year, month, date.day, time.hours, time.minutes)
    return revolver
  ...
  def iso_date(self):
    ...
    temp_date = datetime.datetime(year=year, month=9, day=21, tzinfo=datetime.timezone.utc)
    ...
    for century in range(18, int(temp_date.year / 100)):
        if century % 4 == 0:
            continue
        if temp_date > datetime.datetime(year=temp_date.year, month=2, day=28, tzinfo=datetime.timezone.utc):
            temp_date += datetime.timedelta(days=1)

    # Compute the time (accurate to 1 minute because we don't keep the seconds in the format)
    minutes = (self.hour * 2.4 * 60) + (self.minute * 1.44)
    hour = int(minutes / 60)
    minutes = int(minutes) % 60
    date = datetime.datetime(year=year, month=temp_date.month, day=temp_date.day, hour=hour, minute=minutes, tzinfo=datetime.timezone.utc)
